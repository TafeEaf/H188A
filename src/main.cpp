...
#define ssid      "ilgiovagabondo"        // WiFi SSID
#define password  "letmein1985"    // WiFi password
...
void setup() {
  Serial.begin ( 115200 );
  Serial.println ( "I am at the setup" );
  for ( int x = 0 ; x < 5 ; x++ ) {
...
void loop() {
  ...
  */
  ...
  pa = 1000;
  if ( isnan(t) || isnan(h) ) {
    Serial.println("Error, no valid value");//Erreur, aucune valeur valide - Error, no valid value
  } else {
    addPtToHist();
  }
  delay(5);
